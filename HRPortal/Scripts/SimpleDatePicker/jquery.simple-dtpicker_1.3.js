...
(function ($) {
	...
	**/
    ...
    PickerHandler.prototype.show = function () {
        ...
        //設定html與body的css，為因應iOS捲動背景的問題
        if (!($('body').hasClass('preventBackgroundScroll'))) {
            $('html').addClass('preventBackgroundScroll');
            $('body').addClass('preventBackgroundScroll');
        }
    ...
    PickerHandler.prototype.hide = function () {
		...
		if (func != null) {
		    //console.log("dtpicker- Call the onHide handler 2");
		    func();
		}
        */

        //移除鎖定背景的Class
        var hasActivePicker = false;
        ...
        if (!hasActivePicker) //沒有作用中的才移除Class
        {
            $('html').removeClass('preventBackgroundScroll');
            $('body').removeClass('preventBackgroundScroll');
        }
    ...
    PickerHandler.prototype._relocate = function () {
        ...
        if ($input != null && $picker.data('isInline') === false) { // Float mode
            //console.log('width=' + $picker.parent().css('width') + ',left=' + $picker.parent().css('left') + ',screenWidth=' + document.body.scrollWidth);
            ...
            //檢查右邊界
            //var pickerWidth = $picker.parent().css('width');
            var pickerWidth = $picker.parent().outerWidth();

            //console.log('pickerWidth=' + pickerWidth);

            //var pickerLeft = $picker.parent().css('left');
            var pickerLeft = $input.offset().left;

            //var pickerLeft = $picker.parent().offset().left;
            //console.log('pickerLeft=' + pickerLeft);

            var screenWidth = document.body.scrollWidth;
            //console.log((parseFloat(pickerLeft) + parseFloat(pickerWidth)));
            //console.log( parseFloat(screenWidth));
            if ((parseFloat(pickerLeft) + parseFloat(pickerWidth)) > parseFloat(screenWidth)) {
                $picker.parent().css('left', screenWidth - (pickerWidth + 5));
                //$picker.css('left', screenWidth - (pickerWidth + 5));
                //console.log('A=' + $picker.parent().css('left'));
            }
            else {
	...
	**/
    ...
    var draw = function ($picker, option, year, month, day, hour, min) {
        ...
        for (var zz = 0; i < cellNum; i++) {
            ...
            if (
                ...
                $td.click(function (ev) {
                    ...
                    var flagOutputToInput = $picker.data("dateOnly") ? true : false;
                    //console.log('flagOutputToInput=' + flagOutputToInput);
                    draw($picker, {
                        "isAnim": false,
                        "isOutputToInputObject": true
                        //"isOutputToInputObject": flagOutputToInput //原本只有僅日期選擇器選日期才立即更新textbox，改為連有時間選擇器時選日期都會立即更新textbox
                    ...
                    if ($picker.data("dateOnly") === true && $picker.data("isInline") === false && $picker.data("closeOnSelected")) {
                        ...
                        var func = $picker.data('onHide');
                        if (func != null) {
                            console.log("dtpicker- Call the onHide handler 1");
                            func();
                        }
        ...
        if ($picker.data("dateOnly") === true) {
            ...
            /* Timelist ----- */
            var timelist_original_height = parseInt($timelist.css("height"));
            $timelist.children().remove();

            /* Set height to Timelist (Calendar innerHeight - Calendar padding) */
            if ($calendar.innerHeight() > 0) {
                //$timelist.css("height", $calendar.innerHeight() - 10 + 'px');
                $timelist.css("height", $calendar.outerHeight() + 'px');
            }
            else {
                $timelist.css("height", timelist_original_height + 'px');
            }

            realDayObj = new Date(date.getTime());
            //$timelist.css("height", $calendar.innerHeight() - 10 + 'px');
            //$timelist.css("height", $calendar.innerHeight() + 'px');
            //console.log($calendar.innerHeight() + 'px');
    ...
    var init = function ($obj, opt) {
        ...
        $picker.append($inner);
        var closeWindowText = opt.locale == 'en' ? 'Close Calendar' : '關閉視窗';
        $picker.append($('<div class="btn_close_datetimepicker close_datetimepicker">' + closeWindowText + '<div class="datetimepicker_icon_right close_datetimepicker">&#10006</div></div>'));
        ...
        */
        ...
        $picker.on("click", function (ev) {
            //console.log("picker clicked... event.target is " + ev.target);
            ev.stopPropagation();
            //console.log(ev.target.className + '---' + ev.currentTarget.className);
            if ($(ev.target).hasClass('close_datetimepicker')) {
                $picker.hide();
        ...
        return this.each(function (i) {
            ...
            if (options.inline === true) {
                ...
                if (options.width) {
                    var inputW;
                    var pickerW;
                    if (options.width.substr(options.width.length - 2) == 'px') {
                        pickerW = options.width;
                    }
                    else {

                        //$picker.css("width","auto");
                        inputW = $(input).css('width');
                        pickerW = inputW * (parseFloat(options.width) / 100.0);

                    }
                    $picker.parent().css("width", pickerW);
                    $picker.css("width", pickerW);
                }
                else {
                    $picker.css("width", "auto");
                }
    ...
    var methods = {
        ...
        hide: function () {
            ...
            if ($picker != null) {
                var handler = new PickerHandler($picker, $input);
                // Hide a picker
                //console.log('date=' + getPickedDate($picker) + '，inputDate=' + $input.val());
                handler.hide();
            }
        },
    ...
    $(function () {
        ...
        */
        $('body').click(function (event) {
            ...
            //console.log($(event.target).parents().hasClass("datepicker"));
            for (var i = 0; i < PickerObjects.length; i++) {
                var $picker = $(PickerObjects[i]);
                ...
                if ($picker.data("inputObjectId") != null && $picker.data("isInline") === false && $picker.css('display') != 'none') {
                    ...
                    if ($($input).is(':focus')) continue; //patch
                    var handler = new PickerHandler($picker, $input);


                    //如果隱藏日期元件時，選取的資料與input資料不同，就要更新input資料
                    var locale = $picker.data("locale");
                    var format = getDateFormat($picker.data("dateFormat"), locale, $picker.data('dateOnly'));
                    //console.log('date=' + getFormattedDate(getPickedDate($picker), format) + '，inputDate=' + $($input).val());

                    var pickerDate = getFormattedDate(getPickedDate($picker), format);
                    var inputDate = $($input).val();
                    if (pickerDate != inputDate) {
                        $($input).val(pickerDate);
                        //console.log('different');

                        //現在關閉都會自動更新，不需要額外trigger了
