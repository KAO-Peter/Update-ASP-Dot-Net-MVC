...
{
    public class PfaCycleThirdEvaluationController : BaseController
    {
        const string btnReject = "btnReject";
        const string btnSent = "btnSent";
        const string btnSave = "btnSave";

        #region 查詢
        public ActionResult Index(string txtOrgID = ""
            , string txtDepartmentID = ""
            , string txtEmployeeNo = ""
            , string txtEmployeeName = ""
            , string txtStatus = ""
            , string txtPerformanceID = ""
            , string txtSort = ""
            , string txtNoRatio = ""
            , string txtDeptSort = ""
            , int page = 1
            , string cmd = "")
        {
            ...
            .Where(x => x.IsThirdEvaluation == true 
                && x.PfaCycleEmp.Status != PfaCycleEmp_Status.Locked);
            ...
            if (!string.IsNullOrEmpty(txtPerformanceID))
            {
                ...
                queryData = queryData.Where(x => x.PfaCycleEmp.FinalPerformance_ID == performanceID);
            }
            ...
            var tempQueryData = queryData.Select(x => new PfaCycleEmpSignViewModel()
            {
                ...
                PfaFinalScore = x.PfaCycleEmp.PfaFinalScore,
                LastPerformance_ID = x.PfaCycleEmp.LastPerformance_ID,
                FinalPerformance_ID = x.PfaCycleEmp.FinalPerformance_ID,
                SignStatus = x.Status,
            });
            ...
            {
                SortOption sortOption = SortOption.FirstH;
                if (txtSort == SortOption.FirstH.ToString())
                    sortOption = SortOption.FirstH;
                else if (txtSort == SortOption.FirstL.ToString())
                    sortOption = SortOption.FirstL;
                else if (txtSort == SortOption.SecondH.ToString())
                    sortOption = SortOption.SecondH;
                else if (txtSort == SortOption.SecondL.ToString())
                    sortOption = SortOption.SecondL;
                else if (txtSort == SortOption.ThirdH.ToString())
                    sortOption = SortOption.ThirdH;
                else if (txtSort == SortOption.ThirdL.ToString())
                    sortOption = SortOption.ThirdL;

                switch (sortOption)
                {
                    case SortOption.FirstH:
                    ...
                    case SortOption.SecondL:
                        ...
                        break;
                    case SortOption.ThirdH:
                        tempQueryData = tempQueryData.OrderBy(x => x.PfaCycleID).ThenBy(x => x.PfaOrgCode).ThenBy(x => x.PfaDeptCode).ThenByDescending(x => x.PfaFinalScore).ThenBy(x => x.EmployeeNo);
                        break;
                    case SortOption.ThirdL:
                        tempQueryData = tempQueryData.OrderBy(x => x.PfaCycleID).ThenBy(x => x.PfaOrgCode).ThenBy(x => x.PfaDeptCode).ThenBy(x => x.PfaFinalScore).ThenBy(x => x.EmployeeNo);
                        break;
                }
            }
            else if (txtDeptSort == "Y" && txtSort == "ALL")
            {
                tempQueryData = tempQueryData.OrderBy(x => x.PfaCycleID)
                    .ThenBy(x => x.PfaOrgCode)
                    .ThenBy(x => x.PfaDeptCode)
            ...
            else if (txtDeptSort == "N" && txtSort != "ALL")
            {
                ...
                switch (sortOption)
                {
                    ...
                    case SortOption.SecondL:
                        ...
                        break;
                    case SortOption.ThirdH:
                        tempQueryData = tempQueryData.OrderBy(x => x.PfaCycleID).ThenByDescending(x => x.PfaFinalScore).ThenBy(x => x.EmployeeNo);
                        break;
                    case SortOption.ThirdL:
                        tempQueryData = tempQueryData.OrderBy(x => x.PfaCycleID).ThenBy(x => x.PfaFinalScore).ThenBy(x => x.EmployeeNo);
                        break;
                }
            }
            else
            ...
            .Where(x => pfaCycleIDs.Contains(x.ID))
            .ToList();

            var pfaPerformanceList = Services.GetService<PfaPerformanceService>().GetAll()
            .Where(x => x.CompanyID == CurrentUser.CompanyID)
            .ToList();

            var pfaOptionList = Services.GetService<PfaOptionService>().GetAll()
            .Where(x => x.PfaOptionGroup.OptionGroupCode == PfaOptionGroup_OptionGroupCode.SignStatus)
            .ToList();
            ...
            foreach (var item in viewModel)
            {
                ...
                var pfaPerformance = pfaPerformanceList.FirstOrDefault(y => y.ID == item.FinalPerformance_ID);
                item.FinalPerformanceName = pfaPerformance != null ? pfaPerformance.Name : "";
            }
        ...
        public ActionResult Edit(Guid? Id)
        {
            ...
            var pfaSignProcess = Services.GetService<PfaSignProcessService>().GetAll()
            ...
            PfaCycleEmpSignViewModel model = GetDetail(pfaSignProcess, pfaCycle);
        ...
        public ActionResult Edit(PfaCycleEmpSignViewModel model, string cmd)
        {
            ...
            try
            {
                ...
                var pfaSignProcess = Services.GetService<PfaSignProcessService>().GetAll()
                .FirstOrDefault(x => x.ID == model.PfaSignProcessID);
                if (pfaSignProcess == null)
                    throw new Exception("績效考核簽核資料取得失敗");
                model.EditFirstEvaluation = pfaSignProcess.IsFirstEvaluation;

                var pfaCycleEmp = Services.GetService<PfaCycleEmpService>().GetAll()
                .FirstOrDefault(x => x.ID == model.PfaCycleEmpID);
                if (pfaCycleEmp == null)
                ...
                if (model.EditFirstEvaluation)
                {
                    ...
                    #region 績效考核員工勝任能力
                    var pfaAbilityList = Services.GetService<PfaAbilityService>().GetAll()
                    .Where(x => x.CompanyID == pfaCycle.CompanyID 
                        && x.IsUsed == true 
                        && x.PfaEmpTypeID == pfaSignProcess.PfaCycleEmp.PfaEmpTypeID)
                    ...
                    foreach (var pfaAbility in pfaAbilityList)
                    {
                        ...
                        var tempPfaEmpAbility = model.PfaEmpAbility
                            .FirstOrDefault(x => x.PfaAbilityCode == pfaAbility.PfaAbilityCode);
                        ...
                        foreach (var pfaAbilityDetail in pfaAbilityDetailList)
                        {
                            ...
                            {
                                tempPfaEmpAbilityDetail = tempPfaEmpAbility.PfaEmpAbilityDetail
                                    .FirstOrDefault(x => x.Ordering == pfaAbilityDetail.Ordering);
                            }
                ...
                pfaCycleEmp.PfaFinalScore = model.PfaFinalScore;
                pfaCycleEmp.FinalPerformance_ID = model.FinalPerformance_ID;
                pfaCycleEmp.FinalAppraisal = model.FinalAppraisal;
                ...
                result = Services.GetService<PfaSignProcessService>()
                .UpdateThirdEvaluationData(pfaSignProcess
                    , pfaCycleEmp
                    , pfaEmpIndicatorList
                    , pfaEmpAbilityList
                    , cmd);
        ...
        private Result CheckPfaCycleEmpSign(PfaCycleEmpSignViewModel model, string cmd)
        {
            ...
            try
            {
                ...
                {
                    var checkPfaEmpAbility = pfaEmpAbility.PfaEmpAbilityDetail
                        .OrderBy(x => x.Ordering)
                        .FirstOrDefault(x => !x.AbilityScore.HasValue);
                ...
                model.ManagerAbility = model.PfaEmpAbility
                    .Where(x => x.ManagerAbility.HasValue)
                    .Sum(x => x.ManagerAbility);
                ...
                var pfaPerformanceFirst = Services.GetService<PfaPerformanceService>().GetAll()
                .FirstOrDefault(x => x.CompanyID == CurrentUser.CompanyID
                    && x.IsUsed == true
                    && x.ScoresStart <= tempPfaFirstScore
                    && x.ScoresEnd >= tempPfaFirstScore);
                ...
                if (!model.PfaFinalScore.HasValue)
                {
                    if (cmd == btnSent || model.IsRatio)
                        throw new Exception("請輸入核決總分");
                }
                else
                {
                    if (!Regex.IsMatch(model.PfaFinalScore.Value.ToString(), pattern))
                        throw new Exception("核決總分，小數點最多1位");

                    var tempPfaFinalScore = (decimal)model.PfaFinalScore;
                    var pfaPerformanceLast = Services.GetService<PfaPerformanceService>().GetAll()
                    .FirstOrDefault(x => x.CompanyID == CurrentUser.CompanyID
                        && x.IsUsed == true
                        && x.ScoresStart <= tempPfaFinalScore
                        && x.ScoresEnd >= tempPfaFinalScore);

                    if (pfaPerformanceLast != null)
                        model.FinalPerformance_ID = pfaPerformanceLast.ID;
                    else
                    {
                        if (cmd == btnSent || model.IsRatio)
                            throw new Exception(string.Format("核決總分:{0}，查無績效等第資料", model.PfaFinalScore));
                    }
                }
                ...
                if (cmd == btnSent || model.IsRatio)
                {
                    ...
                    if (string.IsNullOrWhiteSpace(model.FirstAppraisal)
                        && string.IsNullOrWhiteSpace(model.LastAppraisal)
                        && string.IsNullOrWhiteSpace(model.FinalAppraisal))
                        throw new Exception("請輸入核決主管綜合考評");
                }
            }
            catch (Exception ex)
            {
        ...
        private static void SetModelSignStatus(PfaCycleEmpSignViewModel model, string cmd)
        {
            if (cmd == btnReject)
            {
                model.SignStatus = PfaSignProcess_Status.Returned; // b:已退回
            }
            else if (cmd == btnSent)
            {
                model.SignStatus = PfaSignProcess_Status.Submitted; // e: 已送出
            }
            else
            {
                if(model.SignStatus == PfaSignProcess_Status.PendingReview 
                    || model.SignStatus == PfaSignProcess_Status.PendingThirdReview
                    || model.SignStatus == PfaSignProcess_Status.ReturnedForModification)
                {
                    model.SignStatus = PfaSignProcess_Status.Reviewed;
                }
                else
                {
                    model.SignStatus = model.SignStatus;
                }
            }
        }
        #endregion
            ...
            try
            {
                ...
                if (pfaCycleEmpList.Any())
                {
                    ...
                    foreach (var pfaCycleOrgSent in pfaCycleOrgSentList)
                    {
                        ...
                        {
                            if (PFA_Common.CycleRationDetail.CheckThirdEvaluationRatio(pfaCycleRation.OrgTotal
        ...
        public ActionResult SentConfirm(PfaCycleSentDataViewModel model)
        {
            ...
            try
            {
                ...
                foreach (var data in newModel)
                {
                    ...
                    if (false == PFA_Common.CycleRationDetail.CheckThirdEvaluationRatio(pfaCycleRation.OrgTotal
                        , pfaCycleRation.PfaCycleRationDetail.ToList()
                        , out totalScore))
                    {
                ...
                result = Services.GetService<PfaSignProcessService>().SentThirdEvaluationData(newModel, CurrentUser.EmployeeID);
                if (!result.success)
                    throw new Exception(result.message);
            }
            catch (Exception ex)
        ...
        private bool CheckHasUnRatioNoSubmit(Guid pfaCycleID, Guid pfaOrgID, Guid selfEmpId, out string msg)
        {
            ...
            var pfaSignProcessList = Services.GetService<PfaSignProcessService>().GetAll()
                ...
                .Where(x => x.PfaCycleEmp.PfaCycleID == pfaCycleID
                    ...
                    && x.OrgSignEmpID != x.PfaCycleEmp.EmployeeID
        ...
        public ActionResult RejectQuery(Guid? Id = null)
        {
            ...
            try
            {
                ...
                if (pfaCycleEmpList.Any())
                {
                    ...
                    foreach (var pfaCycleDeptSent in pfaCycleDeptSentList)
                    {
                        ...
                        pfaCycleDeptSent.FirstFinal = pfaSignProcessList
                            .Where(x => x.Status == PfaSignProcess_Status.Reviewed
                                || x.Status == PfaSignProcess_Status.Submitted)
                            .Select(x => x.PfaCycleEmpID)
                            .Distinct()
                            .Count();
                        ...
                        {
                            if (pfaSignProcessList.Any(x => x.Status == PfaSignProcess_Status.Reviewed))
                ...
                if (ds.PfaCycleDeptSent.Any(x => x.SignStatus == x.SignStatus_N) == false)
                    ViewBag.Status = "Y";
                else
                {
        ...
        public ActionResult RejectConfirm(PfaCycleSentDataViewModel model)
        {
            ...
            try
            {
                ...
                result = Services.GetService<PfaSignProcessService>().BackThirdEvaluationData(newModel, model.Assessment, CurrentUser.EmployeeID);
                if (!result.success)
                    throw new Exception(result.message);
            }
            catch (Exception ex)
        ...
        public ActionResult LastPerformanceRation(PfaCycleEmpSignViewModel model)
        {
            ...
            try
            {
                ...
                var tempPfaFinalScore = (decimal)model.PfaFinalScore;
                var tempPfaPerformance = Services.GetService<PfaPerformanceService>()
                    .FirstOrDefault(x => x.CompanyID == CurrentUser.CompanyID 
                        && x.ScoresStart <= tempPfaFinalScore 
                        && x.ScoresEnd >= tempPfaFinalScore);
                if (tempPfaPerformance != null)
                    model.FinalPerformance_ID = tempPfaPerformance.ID;
                else
                ...
                var pfaCycleEmpList = Services.GetService<PfaCycleEmpService>().GetAll()
                .Where(x => x.PfaOrgID == pfaCycleEmp.PfaOrgID 
                    && x.PfaCycleID == model.PfaCycleID 
                    && x.IsRatio)
                .Include(r=>r.PfaDept)
                .ToList();

                var pfaDeptIDs = pfaCycleEmpList.Select(x => x.PfaDeptID).Distinct().ToList();
                var pfaDeptList = pfaCycleEmpList
                    .Select(r=>r.PfaDept)
                    .GroupBy(r=>r.ID)
                    .Where(x => pfaDeptIDs.Contains(x.Key))
                    .Select(r=>r.First())
                    .OrderBy(x => x.PfaDeptCode)
                    .ToList();
                ...
                {
                    model.PfaCycleRationData.PfaPerformance = Services.GetService<PfaPerformanceService>()
                    .GetAll()
                    .Where(x => x.CompanyID == CurrentUser.CompanyID && x.IsUsed)
                    .OrderBy(x => x.Ordering)
                    .ToList()
                    .Select(x =>
                    {
                ...
                if (pfaCycle.PfaCycleRation.Any())
                {
                    ...
                    if (pfaCycleRation != null)
                    {
                        ...
                        for (int i = 0; i < RowTypes.Items.Length; i++)
                        {
                            totalScore = 0;
                            totalNum = 0;
                            var tempPfaCycleRationDetailList = model.PfaCycleRationData.PfaCycleRationDetail
                            .Where(x => x.RowType == RowTypes.Items[i])
                            .ToList();
                                        ...
                                        if (tempPfaCycleRationDetail != null && numberData != null)
                                        {
                                            ...
                                            switch (detail.RowType)
                                            {
                                                ...
                                                case RowTypes.First:
                                                    if (detail.PfaDeptID.HasValue)
                                                    {
                                                        var firstFinal = pfaCycleEmpList.Count(x => x.PfaDeptID == detail.PfaDeptID.Value
                                                            && x.FirstPerformance_ID == pfaPerformance.ID);
                                                ...
                                                case RowTypes.Second:
                                                    int secondEmpCount = 
                                                        pfaCycleEmpList.Count(x => x.LastPerformance_ID == pfaPerformance.ID);

                                                    numberData.Number = secondEmpCount;
                                                    if (tempPfaCycleRationDetail.Multiplier.HasValue)
                                                    {
                                                        totalScore += numberData.Number
                                                            * tempPfaCycleRationDetail.Multiplier.Value;
                                                    }
                                                    totalNum += numberData.Number;
                                                    break;
                                                case RowTypes.Last:
                                                case RowTypes.Calculat:
                                                    int finalEmpCount = 
                                                        pfaCycleEmpList.Count(x => x.FinalPerformance_ID == pfaPerformance.ID);

                                                    if (model.FinalPerformance_ID == pfaPerformance.ID)
                                                    {
                                                        finalEmpCount++;
                                                    }
                                                    numberData.Number = finalEmpCount;
                                                    if (tempPfaCycleRationDetail.Multiplier.HasValue)
                                                    {
                                                        totalScore += numberData.Number
                                                            * tempPfaCycleRationDetail.Multiplier.Value;
            ...
            , string cmdSub = "")
        {
            ...
            try
            {
                ...
                List<PfaSignProcess> pfaSignProcessDatas = Services.GetService<PfaSignProcessService>().GetAll()
                    .Where(x => x.PfaCycleEmp.IsRatio
                        && x.PfaCycleEmp.PfaCycleID == pfaCycle.ID
                        && x.IsRatio
                        && x.IsThirdEvaluation == true)
                ...
                // 該主管所有可評核績效考核組織
                IQueryable<PfaOrg> queryOrgData = Services.GetService<PfaOrgService>().GetAll()
                .Where(x => x.CompanyID == CurrentUser.CompanyID);

                if (!CurrentUser.IsAdmin)
                {
                    var pfaOrgIDs = pfaSignProcessDatas
                    .Where(x => x.PreSignEmpID == CurrentUser.EmployeeID)
                    .Select(x => x.PfaCycleEmp.PfaOrgID)
                    .Distinct()
                    .ToList();
                ...
                if (orgList.Any())
                {
                    foreach (var org in orgList)
                        listItem.Add(new SelectListItem
                        {
                            ...
                            , org.PfaOrgCode, org.PfaOrgName)
                            ,
                            Value = org.ID.ToString()
                            ,
                            Selected = (selectedDataID == org.ID ? true : false)
                        });
                }
                ...
                if (orgList.Any())
                {
                    ...
                    var pfaCycleEmpList = Services.GetService<PfaCycleEmpService>().GetAll()
                    .Where(x => x.PfaOrgID == model.PfaOrgID
                        && x.PfaCycleID == model.PfaCycleID
                        && x.IsRatio).ToList();

                    var pfaDeptIDs = pfaCycleEmpList.Select(x => x.PfaDeptID).Distinct().ToList();

                    var pfaDeptList = Services.GetService<PfaDeptService>().GetAll()
                    .Where(x => pfaDeptIDs.Contains(x.ID))
                    .OrderBy(x => x.PfaDeptCode)
                    .ToList();
                    ...
                    if (pfaCycleRationFlag == false)
                    {
                        model.PfaCycleRationData.PfaPerformance = Services.GetService<PfaPerformanceService>().GetAll()
                        .Where(x => x.CompanyID == CurrentUser.CompanyID
                            && x.IsUsed)
                        .OrderBy(x => x.Ordering)
                        .ToList()
                    ...
                    if (pfaCycle.PfaCycleRation.Any())
                    {
                        ...
                        if (pfaCycleRation != null)
                        {
                            ...
                            for (int i = 0; i < RowTypes.Items.Length; i++)
                            {
                                totalScore = 0;
                                totalNum = 0;
                                var tempPfaCycleRationDetailList = model.PfaCycleRationData
                                .PfaCycleRationDetail
                                .Where(x => x.RowType == RowTypes.Items[i])
                                .ToList();
                                ...
                                foreach (var detail in tempPfaCycleRationDetailList)
                                {
                                    ...
                                    if (model.PfaCycleRationData.PfaPerformance.Any())
                                    {
                                        foreach (var pfaPerformance in model.PfaCycleRationData.PfaPerformance)
                                        {
                                            var tempPfaCycleRationDetail = pfaCycleRationDetailList
                                            .FirstOrDefault(x => x.PfaPerformanceID == pfaPerformance.ID);
                                            ...
                                            if (tempPfaCycleRationDetail != null && numberData != null)
                                            {
                                                switch (detail.RowType)
                                                {
                                                    ...
                                                    case RowTypes.First:
                                                        if (detail.PfaDeptID.HasValue)
                                                        {
                                                            var firstFinal = pfaCycleEmpList
                                                            .Count(x => x.PfaDeptID == detail.PfaDeptID.Value
                                                                && x.FirstPerformance_ID == pfaPerformance.ID);
                ...
                #region 考核人員資料
                var queryData = Services.GetService<PfaSignProcessService>().GetAll()
                .Where(x => x.PfaCycleEmp.PfaFirstScore.HasValue
                    && (x.Status == PfaSignProcess_Status.PendingReview 
                        || x.Status == PfaSignProcess_Status.PendingThirdReview
                        || x.Status == PfaSignProcess_Status.Reviewed
                        || x.Status == PfaSignProcess_Status.ReturnedForModification
                ...
                {
                    SortOption sortOption = SortOption.FirstH;
                    if (txtSortSub == SortOption.FirstH.ToString())
                        sortOption = SortOption.FirstH;
                    else if (txtSortSub == SortOption.FirstL.ToString())
                        sortOption = SortOption.FirstL;
                    else if (txtSortSub == SortOption.SecondH.ToString())
                        sortOption = SortOption.SecondH;
                    else if (txtSortSub == SortOption.SecondL.ToString())
                        sortOption = SortOption.SecondL;
                    else if (txtSortSub == SortOption.ThirdH.ToString())
                        sortOption = SortOption.ThirdH;
                    else if (txtSortSub == SortOption.ThirdL.ToString())
                        sortOption = SortOption.ThirdL;

                    switch (sortOption)
                    {
                        case SortOption.FirstH: //初核分數降冪
                            tempQueryData = tempQueryData.OrderBy(x => x.PfaCycleID).ThenBy(x => x.PfaOrgCode).ThenBy(x => x.PfaDeptCode).ThenByDescending(x => x.PfaFirstScore).ThenBy(x => x.EmployeeNo);
                            break;
                        case SortOption.FirstL: //初核分數升冪
                            tempQueryData = tempQueryData.OrderBy(x => x.PfaCycleID).ThenBy(x => x.PfaOrgCode).ThenBy(x => x.PfaDeptCode).ThenBy(x => x.PfaFirstScore).ThenBy(x => x.EmployeeNo);
                            break;
                        case SortOption.SecondH: //複核分數降冪
                            tempQueryData = tempQueryData.OrderBy(x => x.PfaCycleID).ThenBy(x => x.PfaOrgCode).ThenBy(x => x.PfaDeptCode).ThenByDescending(x => x.PfaLastScore).ThenBy(x => x.EmployeeNo);
                            break;
                        case SortOption.SecondL: //複核分數升冪
                            tempQueryData = tempQueryData.OrderBy(x => x.PfaCycleID).ThenBy(x => x.PfaOrgCode).ThenBy(x => x.PfaDeptCode).ThenBy(x => x.PfaLastScore).ThenBy(x => x.EmployeeNo);
                            break;
                        case SortOption.ThirdH:
                            tempQueryData = tempQueryData.OrderBy(x => x.PfaCycleID).ThenBy(x => x.PfaOrgCode).ThenBy(x => x.PfaDeptCode).ThenByDescending(x => x.PfaFinalScore).ThenBy(x => x.EmployeeNo);
            ...
            try
            {
                ...
                foreach (var emp in data.PfaCycleEmp)
                {
                    if (emp.PfaLastScore.HasValue)
                    {
                        ...
                        var tempPfaPerformance = Services.GetService<PfaPerformanceService>()
                        .FirstOrDefault(x => x.CompanyID == CurrentUser.CompanyID
                            && x.ScoresStart <= tempPfaLastScore
                            && x.ScoresEnd >= tempPfaLastScore);

                        if (tempPfaPerformance != null)
                            emp.FinalPerformance_ID = tempPfaPerformance.ID;
                        else
                ...
                if (pfaCycleRationFlag == false)
                {
                    model.PfaCycleRationData.PfaPerformance =
                    Services.GetService<PfaPerformanceService>()
                    .GetAll()
                    .Where(x => x.CompanyID == CurrentUser.CompanyID && x.IsUsed)
                    .OrderBy(x => x.Ordering).
                    ToList()
            ...
            try
            {
                ...
                foreach (var item in model.PfaCycleEmp)
                {
                    ...
                    try
                    {
                        ...
                        var pfaSignProcess = Services.GetService<PfaSignProcessService>().GetAll()
                        .FirstOrDefault(x => x.ID == item.PfaSignProcessID);

                        var detail = GetDetail(pfaSignProcess, pfaCycle);
                        detail.PfaFinalScore = item.PfaFinalScore;

                        SetModelSignStatus(detail, btnSave);

                        var checkResult = CheckPfaCycleEmpSign(detail, btnSave);
                        if (!checkResult.success)
                            throw new Exception(string.Format("{0}編輯錯誤:{1}", errMsg, checkResult.message));

                        pfaCycleEmp.PfaFinalScore = detail.PfaFinalScore;
                        pfaCycleEmp.FinalPerformance_ID = detail.FinalPerformance_ID;
                        pfaCycleEmp.ModifiedBy = CurrentUser.EmployeeID;
                        pfaCycleEmp.ModifiedTime = DateTime.Now;

                        pfaSignProcess.Status = detail.SignStatus;

                        var updateResult = Services.GetService<PfaSignProcessService>()
                        .UpdateThirdEvaluationData(pfaSignProcess
                            , pfaCycleEmp
                            , new List<PfaEmpIndicator>()
                            , new List<PfaEmpAbility>()
                            , btnSave);
        ...
        private PfaCycleEmpSignViewModel GetDetail(PfaSignProcess pfaSignProcess, PfaCycle pfaCycle)
        {
            ...
            var model = new PfaCycleEmpSignViewModel
            {
                ...
                FinalAppraisal = pfaSignProcess.PfaCycleEmp.FinalAppraisal,
                PfaFirstScore = pfaSignProcess.PfaCycleEmp.PfaFirstScore,
                PfaLastScore = pfaSignProcess.PfaCycleEmp.PfaLastScore,
                PfaFinalScore = pfaSignProcess.PfaCycleEmp.PfaFinalScore,
            ...
            var hire = Services.GetService<PfaOptionService>()
                .GetPfaOptionByGroupCode(PfaOptionGroup_OptionGroupCode.Hire)
                .FirstOrDefault(x => x.ID == pfaSignProcess.PfaCycleEmp.HireID);
            model.HireCode = hire != null ? hire.OptionCode : "";
            model.HireName = hire != null ? hire.OptionName : "";

            var jobTitle = Services.GetService<PfaOptionService>()
                .GetPfaOptionByGroupCode(PfaOptionGroup_OptionGroupCode.JobTitle)
                .FirstOrDefault(x => x.ID == pfaSignProcess.PfaCycleEmp.JobTitleID);
            model.JobTitleCode = jobTitle != null ? jobTitle.OptionCode : "";
            model.JobTitleName = jobTitle != null ? jobTitle.OptionName : "";

            var jobFunction = Services.GetService<PfaOptionService>()
                .GetPfaOptionByGroupCode(PfaOptionGroup_OptionGroupCode.JobFunction)
                .FirstOrDefault(x => x.ID == pfaSignProcess.PfaCycleEmp.JobFunctionID);
            model.JobFunctionCode = jobFunction != null ? jobFunction.OptionCode : "";
            model.JobFunctionName = jobFunction != null ? jobFunction.OptionName : "";

            var grade = Services.GetService<PfaOptionService>()
                .GetPfaOptionByGroupCode(PfaOptionGroup_OptionGroupCode.Grade)
                .FirstOrDefault(x => x.ID == pfaSignProcess.PfaCycleEmp.GradeID);
            model.GradeCode = grade != null ? grade.OptionCode : "";
            model.GradeName = grade != null ? grade.OptionName : "";
            ...
            else
            {
                ...
                var pfaIndicatorList = Services.GetService<PfaIndicatorService>().GetAll()
                .Where(x => x.CompanyID == pfaCycle.CompanyID && x.IsUsed == true)
                .OrderBy(x => x.Ordering)
                .ToList();
            ...
            model.PfaSignUploadData.PfaSignUpload = Services.GetService<PfaSignUploadService>().GetAll()
                .Where(x => x.PfaCycleEmpID == pfaSignProcess.PfaCycleEmp.ID)
                .OrderBy(x => x.Ordering)
                .ToList()
                .Select(x =>
                {
            ...
            var pfaCycleEmpList = Services.GetService<PfaCycleEmpService>().GetAll()
            .Where(x => x.PfaOrgID == pfaSignProcess.PfaCycleEmp.PfaOrgID
                && x.PfaCycleID == model.PfaCycleID
                && x.IsRatio)
            .ToList();
            var pfaDeptIDs = pfaCycleEmpList.Select(x => x.PfaDeptID).Distinct().ToList();
                                    ...
                                    if (tempPfaCycleRationDetail != null && numberData != null)
                                    {
                                        ...
                                        switch (detail.RowType)
                                        {
                                            ...
                                            case RowTypes.First:
                                                ...
                                                break;
                                            case RowTypes.Second:
                                                int secondEmpCount = pfaCycleEmpList.Count(x => x.LastPerformance_ID == pfaPerformance.ID);
                                                numberData.Number = secondEmpCount;
                                                if (tempPfaCycleRationDetail.Multiplier.HasValue)
                                                {
                                                    totalScore += numberData.Number
                                            ...
                                            case RowTypes.Last:
                                            case RowTypes.Calculat:
                                                int finalEmpCount = pfaCycleEmpList.Count(x => x.FinalPerformance_ID == pfaPerformance.ID);
                                                numberData.Number = finalEmpCount;
                                                if (tempPfaCycleRationDetail.Multiplier.HasValue)
                                                {
                                                    totalScore += numberData.Number
        ...
        #region 初始值
        private void GetDefaultData(string txtOrgID = ""
            , string txtDepartmentID = ""
            , string txtEmployeeNo = ""
            , string txtEmployeeName = ""
            , string txtStatus = ""
            ...
            , string txtDeptSort = "")
        {
            ...
            ViewBag.StatusList = Common.PFA.PfaOption.GetPfaSignStatusOption(txtStatus, flag: true);
        ...
        public ActionResult GetDepartmentOptions(string selectedData)
        {
            ...
            try
            {
                ...
                if (!string.IsNullOrWhiteSpace(selectedData))
                {
                    ...
                    pfaDeptIDs = Services.GetService<PfaOrgDeptService>().GetAll()
                    .Where(x => x.PfaOrg.CompanyID == CurrentUser.CompanyID && x.PfaOrgID == SelectedDataID)
                    .Select(x => x.PfaDeptID)
                    .Distinct()
                    .ToList();
                }
                else
                {
                    ...
                    pfaDeptIDs = Services.GetService<PfaOrgDeptService>().GetAll()
                    .Where(x => x.PfaOrg.CompanyID == CurrentUser.CompanyID 
                        && orgIDs.Contains(x.PfaOrgID))
                    .Select(x => x.PfaDeptID)
                        .Distinct()
                        .ToList();
                }
                var data = Services.GetService<PfaDeptService>().GetAll()
                .Where(x => x.CompanyID == CurrentUser.CompanyID 
                    && pfaDeptIDs.Contains(x.ID) 
        ...
        private List<SelectListItem> GetOrgList(string selectedData, bool flag = true)
        {
            ...
            try
            {
                ...
                var pfaCycleList = Services.GetService<PfaCycleService>().GetAll()
                .Where(x => x.CompanyID == CurrentUser.CompanyID && x.Status == "a")
                .ToList()
                .OrderByDescending(x => x.PfaYear)
                .ToList();
                ...
                foreach (var item in data)
                {
                    listItem.Add(new SelectListItem { Text = string.Format("{0} {1}"
                        , item.PfaOrgCode, item.PfaOrgName)
                        , Value = item.ID.ToString()
                        , Selected = (SelectedDataID == item.ID ? true : false) 
                    });
                }
            }
            catch {
        ...
        private List<SelectListItem> GetDepartmentList(string selectedData, List<SelectListItem> orgList)
        {
            ...
            var pfaDeptIDs = Services.GetService<PfaOrgDeptService>().GetAll()
            .Where(x => x.PfaOrg.CompanyID == CurrentUser.CompanyID 
                && orgIDs.Contains(x.PfaOrgID))
            .Select(x=> x.PfaDeptID)
            .Distinct()
            ...
            foreach (var item in data)
                listItem.Add(new SelectListItem { Text = string.Format("{0} {1}"
                    , item.PfaDeptCode, item.PfaDeptName)
                    , Value = item.ID.ToString()
                    , Selected = (SelectedDataID == item.ID ? true : false) });
        ...
        {
            var listItem = new List<SelectListItem>();
            var data = Services.GetService<PfaOptionService>().GetAll()
            .Where(x => x.PfaOptionGroup.OptionGroupCode == groupCode)
            .OrderBy(x => x.Ordering)
            .ToList();
            ...
            if (valueFlag == "ID")
            {
                ...
                foreach (var item in data)
                    listItem.Add(new SelectListItem { Text = item.OptionName
                        , Value = item.ID.ToString()
                        , Selected = (SelectedDataID == item.ID ? true : false) });
            }
            else
        ...
        private List<SelectListItem> GetPfaFormNoList(List<PfaCycle> datas, string selectedData)
        {
            ...
            foreach (var item in datas.OrderByDescending(r => r.PfaYear).ThenByDescending(r => r.PfaFormNo))
                listItem.Add(new SelectListItem { Text = item.PfaFormNo
                    , Value = item.ID.ToString()
                    , Selected = (SelectedDataID == item.ID ? true : false) });
            return listItem;
        }
        ...
        private List<SelectListItem> GetPerformanceList(string selectedData)
        {
            ...
            var data = Services.GetService<PfaPerformanceService>().GetAll()
            .Where(x => x.CompanyID == CurrentUser.CompanyID 
                && x.IsUsed)
            .OrderBy(x => x.Ordering)
            .ToList();
            ...
            foreach (var item in data)
                listItem.Add(new SelectListItem { Text = item.Name
                    , Value = item.ID.ToString()
                    , Selected = (SelectedDataID == item.ID ? true : false) });
        ...
        private List<SelectListItem> GetSortList(string selectedData)
        {
            ...
            {
                new SelectListItem { Text = "空白", Value = "ALL", Selected = (selectedData == "ALL") },
                new SelectListItem { Text = "初核分數降冪", Value = SortOption.FirstH.ToString()
                , Selected = (selectedData == SortOption.FirstH.ToString()) },

                new SelectListItem { Text = "初核分數升冪", Value = SortOption.FirstL.ToString()
                , Selected = (selectedData == SortOption.FirstL.ToString()) },

                new SelectListItem { Text = "複核分數降冪", Value = SortOption.SecondH.ToString()
                , Selected = (selectedData == SortOption.SecondH.ToString()) },

                new SelectListItem { Text = "複核分數升冪", Value = SortOption.SecondL.ToString()
                , Selected = (selectedData == SortOption.SecondL.ToString()) },

                new SelectListItem { Text = "核決分數降冪", Value = SortOption.ThirdH.ToString()
                , Selected = (selectedData == SortOption.ThirdH.ToString()) },

                new SelectListItem { Text = "核決分數升冪", Value = SortOption.ThirdL.ToString()
                , Selected = (selectedData == SortOption.ThirdL.ToString()) }
    ...
    public class RowTypes
    {
        ...
        public const string Calculat = "Calculat";

        public const string itemNameRation = "配比";
        public const string itemNameFirst = "初核";
        public const string itemNameSecond = "複核";
        public const string itemNameLast = "核決";
        public const string itemNameCalculat = "試算";
        public static string[] Items = { Ration, First, Second, Last, Calculat };
        public static string[] ItemNames = { itemNameRation, itemNameFirst, itemNameSecond, itemNameLast, itemNameCalculat };
    }
}