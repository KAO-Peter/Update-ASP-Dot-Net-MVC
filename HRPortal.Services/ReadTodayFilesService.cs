...
{
    public class ReadTodayFilesService : BaseCrudService<ReadTodayFiles>
    {

        public ReadTodayFilesService(HRPortal_Services services)
            : base(services)
        {
        }
        ...
        public ReadTodayFiles GetFileByAnnouncemrnyID(Guid ID)
        {
            var data = GetAll().Where(x => x.AnnouncementID == ID).FirstOrDefault();
        ...
        public override int Create(ReadTodayFiles model, bool isSave = true)
        {
            var _result = 0;
            try
            {
        ...
        public override int Update(ReadTodayFiles oldData, ReadTodayFiles newData, string[] includeProperties, bool isSave = true)
        {
            int result = 0;
            try
            {
